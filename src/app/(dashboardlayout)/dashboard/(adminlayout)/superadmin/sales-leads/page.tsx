...
const NormalUsersTable = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const unassignedLeads = result?.data?.leads?.filter((l: any) => !l.Operator) || [];
        ...
        <table className="w-full text-sm text-left border border-gray-200">
            ...
            <tbody>
              {(paginated as FollowUpRecord[]).map((record, index) => (
                  ...
                  <td className="p-3">{record.leadDetails.CustomerName}</td>
                  <td className="p-3">{record.leadDetails.PhoneNumber}</td>
                  <td className="p-3">{record.leadDetails.Operator}</td>
                  <td className="p-3">{record.remarks}</td>
                  <td className="p-3">
                    {record.reminderDateTime?.slice(0, 19).replace("T", " ")}
                  </td>
                  <td className="p-3">
                    {record.createdAt?.slice(0, 19).replace("T", " ")}
                  </td>
