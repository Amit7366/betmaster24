...
export default function DepositPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (parseFloat(Number(form.amount).toFixed(2)) < 300) {
      toast.error("Minimum deposit amount is 300 TK.");
      return;
    }
      ...
      <div className="w-full bg-secondary rounded-2xl shadow-md p-2 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={submitting}
            className={`w-full h-12 mt-2 font-semibold rounded-full transition flex justify-between items-center text-sm px-4 ${
              submitting
