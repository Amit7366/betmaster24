...
export default function AddWalletPage() {
  ...
  const fetchWallets = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/wallets/user/${userInfo?.objectId}`,
        {
          headers: {
  ...
  useEffect(() => {
    if (userInfo?.objectId && authToken) {
      fetchWallets();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const promise = fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/wallets`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <div className="w-full pt-5 pb-12 px-2 flex flex-col gap-5 bg-primary">
      ...
      <div className="w-full bg-secondary rounded-2xl shadow-md p-8">
        <h2 className="text-sm font-semibold mb-6 text-white">Add New Wallet</h2>
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Wallet Name Dropdown */}
          <div>
            <label className="block mb-1 text-sm font-medium text-accent">Wallet Name</label>
            <select
              name="walletName"
          ...
          {/* Account Holder Name */}
          <div>
            <label className="block mb-1 text-sm font-medium text-accent">Account Holder Name</label>
            <input
              type="text"
          ...
          {/* Wallet Number */}
          <div>
            <label className="block mb-1 text-sm font-medium text-accent">Wallet Number</label>
            <input
              type="text"
              name="walletNumber"
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label className="text-sm font-medium text-white">Set as Default Wallet</label>
          </div>
