...
const WithdrawPage = () => {
    ...
    const fetchWallets = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/wallets/user/${objectId}`,
          {
            headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      id: userInfo.id,
      amount: amount,
      paymentMethod: wallets.find((w) => w._id === selectedWalletId)?.walletName || "",
      walletNumber,
      transactionId: "sbm",
      accountHolderName,
    };
    ...
    const promise = fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/transaction/withdraw/manual`,
      {
        method: "POST",
        headers: {
